...
public final class CFMetaData {
	...
	public final static ByteBuffer DEFAULT_KEY_NAME = ByteBufferUtil
			.bytes("KEY");
	...
	public final static SpeculativeRetry DEFAULT_SPECULATIVE_RETRY = new SpeculativeRetry(
			SpeculativeRetry.RetryType.NONE, 0);
	public final static int DEFAULT_INDEX_INTERVAL = 128;
	public final static boolean DEFAULT_POPULATE_IO_CACHE_ON_FLUSH = false;

	// Alex Santos
	// public final static boolean DEFAULT_is_view = false;

	// Note that this is the default only for user created tables
	public final static String DEFAULT_COMPRESSOR = LZ4Compressor.class
			.getCanonicalName();

	@Deprecated
	public static final CFMetaData OldStatusCf = newSystemMetadata(
			Table.SYSTEM_KS, SystemTable.OLD_STATUS_CF, 0, "unused",
			BytesType.instance, null);
	@Deprecated
	public static final CFMetaData OldHintsCf = newSystemMetadata(
			Table.SYSTEM_KS, SystemTable.OLD_HINTS_CF, 1, "unused",
			BytesType.instance, BytesType.instance);
	@Deprecated
	public static final CFMetaData OldMigrationsCf = newSystemMetadata(
			Table.SYSTEM_KS, DefsTable.OLD_MIGRATIONS_CF, 2, "unused",
			TimeUUIDType.instance, null);
	@Deprecated
	public static final CFMetaData OldSchemaCf = newSystemMetadata(
			Table.SYSTEM_KS, DefsTable.OLD_SCHEMA_CF, 3, "unused",
			UTF8Type.instance, null);

	public static final CFMetaData IndexCf = compile(
			5,
			"CREATE TABLE \""
	...
	public static final CFMetaData CounterIdCf = compile(6, "CREATE TABLE \""
			+ SystemTable.COUNTER_ID_CF + "\" (" + "key text," + "id timeuuid,"
			+ "PRIMARY KEY (key, id)"
			+ ") WITH COMPACT STORAGE AND COMMENT='counter node IDs'");

	// new-style schema
	public static final CFMetaData SchemaKeyspacesCf = compile(
			8,
			"CREATE TABLE "
	...
	public static final CFMetaData SchemaColumnFamiliesCf = compile(
			9,
			"CREATE TABLE "
					...
					+ "key_alias text," // that one is kept for compatibility
										// sake
					+ "key_aliases text,"
					...
					+ "is_view text," // Alex Santos
					+ "view_properties text," // Alex Santos
					+ "PRIMARY KEY (keyspace_name, columnfamily_name)"
					+ ") WITH COMMENT='ColumnFamily definitions' AND gc_grace_seconds=8640");
	public static final CFMetaData SchemaColumnsCf = compile(
			10,
			"CREATE TABLE "
	...
	public static final CFMetaData HintsCf = compile(
			11,
			"CREATE TABLE "
	...
	public static final CFMetaData PeersCf = compile(12, "CREATE TABLE "
			+ SystemTable.PEERS_CF + " (" + "peer inet PRIMARY KEY,"
			+ "host_id uuid," + "tokens set<varchar>," + "schema_version uuid,"
	...
	public static final CFMetaData PeerEventsCf = compile(12, "CREATE TABLE "
			+ SystemTable.PEER_EVENTS_CF + " (" + "peer inet PRIMARY KEY,"
			+ "hints_dropped map<uuid, int>"
			+ ") WITH COMMENT='cf contains events related to peers'");

	public static final CFMetaData LocalCf = compile(13, "CREATE TABLE "
			+ SystemTable.LOCAL_CF + " (" + "key text PRIMARY KEY,"
			+ "tokens set<varchar>," + "cluster_name text,"
			+ "gossip_generation int," + "bootstrapped text," + "host_id uuid,"
			+ "release_version text," + "thrift_version text,"
			+ "cql_version text," + "data_center text," + "rack text,"
			+ "partitioner text," + "schema_version uuid,"
			+ "truncated_at map<uuid, blob>"
			+ ") WITH COMMENT='information about the local node'");

	public static final CFMetaData TraceSessionsCf = compile(14,
			"CREATE TABLE " + Tracing.SESSIONS_CF + " ("
					+ "  session_id uuid PRIMARY KEY," + "  coordinator inet,"
					+ "  request text," + "  started_at timestamp,"
					+ "  parameters map<text, text>," + "  duration int"
					+ ") WITH COMMENT='traced sessions'", Tracing.TRACE_KS);

	public static final CFMetaData TraceEventsCf = compile(15, "CREATE TABLE "
			+ Tracing.EVENTS_CF + " (" + "  session_id uuid,"
			+ "  event_id timeuuid," + "  source inet," + "  thread text,"
			+ "  activity text," + "  source_elapsed int,"
			+ "  PRIMARY KEY (session_id, event_id)" + ");", Tracing.TRACE_KS);

	public static final CFMetaData BatchlogCf = compile(
			16,
			"CREATE TABLE "
	...
	public static final CFMetaData RangeXfersCf = compile(17, "CREATE TABLE "
			...
			+ "token_bytes blob PRIMARY KEY," + "requested_at timestamp"
			+ ") WITH COMMENT='ranges requested for transfer here'");

	public static final CFMetaData CompactionLogCf = compile(18,
			"CREATE TABLE " + SystemTable.COMPACTION_LOG + " ("
					+ "id uuid PRIMARY KEY," + "keyspace_name text,"
					+ "columnfamily_name text," + "inputs set<int>"
					+ ") WITH COMMENT='unfinished compactions'");

	public enum Caching {
		ALL, KEYS_ONLY, ROWS_ONLY, NONE;

		public static Caching fromString(String cache)
				throws ConfigurationException {
			try {
				return valueOf(cache.toUpperCase());
			} catch (IllegalArgumentException e) {
				throw new ConfigurationException(String.format(
						"%s not found, available types: %s.", cache,
	...
	public static class SpeculativeRetry {
		...
		public static SpeculativeRetry fromString(String retry)
				throws ConfigurationException {
			String name = retry.toUpperCase();
			try {
				if (name.endsWith(RetryType.PERCENTILE.toString())) {
					long value = Long.parseLong(name.substring(0,
							name.length() - 10));
			...
			throw new ConfigurationException("invalid speculative_retry type: "
					+ retry);
		}
	...
	public volatile AbstractType<?> comparator; // bytes, long, timeuuid, utf8,
												// etc.
	...
	// Alex Santos
	private volatile Map<String, String> isView = new HashMap<String, String>(); // =
	private volatile Map<String, List<String>> viewProperties = new HashMap<String, List<String>>(); 																				// DEFAULT_is_view;

	/*
	 * All CQL3 columns definition are stored in the column_metadata map. On top
	 * of that, we keep separated collection of each kind of definition, to 1)
	 * allow easy access to each kind and 2) for the partition key and
	...
	private volatile List<ColumnDefinition> partitionKeyColumns; // Always of
																	// size
																	// keyValidator.componentsCount,
																	// null
																	// padded if
																	// necessary
	...
	public volatile CompressionParameters compressionParameters = new CompressionParameters(
			null);

	// Processed infos used by CQL. This can be fully reconstructed from the
	// CFMedata,
	// so it's not saved on disk. It is however costlyish to recreate for each
	// query
	...
	public CFMetaData memtableFlushPeriod(int prop) {
		memtableFlushPeriod = prop;
		return this;
	}

	public CFMetaData defaultTimeToLive(int prop) {
		defaultTimeToLive = prop;
		return this;
	}

	public CFMetaData speculativeRetry(SpeculativeRetry prop) {
		speculativeRetry = prop;
		return this;
	}

	public CFMetaData populateIoCacheOnFlush(boolean prop) {
		populateIoCacheOnFlush = prop;
		return this;
	}

	// Alex Santos
	public CFMetaData isView(Map<String, String> prop) {
		isView = prop;
		return this;
	}

	// Alex Santos
	public CFMetaData viewProperties(Map<String, List<String>> prop) {
		viewProperties = prop;
		return this;
	}
	...
	private static CFMetaData compile(int id, String cql, String keyspace) {
		try {
			...
			CFMetaData cfmd = newSystemMetadata(keyspace,
					statement.columnFamily(), id, "", statement.comparator,
					null);
	...
	private static AbstractType<?> makeComparator(ColumnFamilyType cftype,
			AbstractType<?> comp, AbstractType<?> subcc) {
		return cftype == ColumnFamilyType.Super ? CompositeType.getInstance(
				comp, subcc == null ? BytesType.instance : subcc) : comp;
	}
	...
	private static CFMetaData newSystemMetadata(String keyspace, String cfName,
			int oldCfId, String comment, AbstractType<?> comparator,
			AbstractType<?> subcc) {
		ColumnFamilyType type = subcc == null ? ColumnFamilyType.Standard
				: ColumnFamilyType.Super;
		CFMetaData newCFMD = new CFMetaData(keyspace, cfName, type, comparator,
				subcc);

		// adding old -> new style ID mapping to support backward compatibility
		Schema.instance.addOldCfIdMapping(oldCfId, newCFMD.cfId);

		return newCFMD.comment(comment).readRepairChance(0)
				.dcLocalReadRepairChance(0).gcGraceSeconds(0);
	}
	...
	public static CFMetaData newIndexMetadata(CFMetaData parent,
		...
		Caching indexCaching = parent.getCaching() == Caching.ALL
				|| parent.getCaching() == Caching.KEYS_ONLY ? Caching.KEYS_ONLY
				: Caching.NONE;

		return new CFMetaData(parent.ksName,
				parent.indexColumnFamilyName(info), ColumnFamilyType.Standard,
				columnComparator, (AbstractType) null)
				.keyValidator(info.getValidator()).readRepairChance(0.0)
				.dcLocalReadRepairChance(0.0).gcGraceSeconds(0)
	...
	static CFMetaData copyOpts(CFMetaData newCFMD, CFMetaData oldCFMD) {
		...
		return newCFMD.comment(oldCFMD.comment)
				...
				.isView(oldCFMD.isView)
				.viewProperties(oldCFMD.viewProperties); // Alex Santos
	}

	/**
	 * generate a column family name for an index corresponding to the given
	 * column. This is NOT the same as the index's name! This is only used in
	 * sstable filenames and is not exposed to users.
	 * 
	 * @param info
	...
	public String indexColumnFamilyName(ColumnDefinition info) {
		...
		return cfName
				...
				+ (info.getIndexName() == null ? ByteBufferUtil
						.bytesToHex(info.name) : info.getIndexName());
	}
	...
	// Alex Santos
	public Map<String, String> isView() {
		return isView;
	}

	// Alex Santos
	public Map<String, List<String>> viewProperties() {
	...
	public ByteBuffer getKeyName() {
		if (partitionKeyColumns.size() > 1)
			throw new IllegalStateException(
					"Cannot acces column family with composite key from CQL < 3.0.0");

		return partitionKeyColumns.get(0) == null ? DEFAULT_KEY_NAME
				: partitionKeyColumns.get(0).name;
	}
	...
	public double getBloomFilterFpChance() {
		...
		// that
		return (bloomFilterFpChance == null || bloomFilterFpChance == 0) ? compactionStrategyClass == LeveledCompactionStrategy.class ? 0.1
				: 0.01
				: bloomFilterFpChance;
	}
	...
	public boolean equals(Object obj) {
		...
		return new EqualsBuilder()
				...
				.append(isView, rhs.isView) // Alex Santos
				.append(viewProperties, rhs.viewProperties)
				.isEquals();
	}

	public int hashCode() {
		return new HashCodeBuilder(29, 1597).append(ksName).append(cfName)
				.append(cfType).append(comparator).append(comment)
				.append(readRepairChance).append(dcLocalReadRepairChance)
				.append(replicateOnWrite).append(gcGraceSeconds)
				.append(defaultValidator).append(keyValidator)
				.append(minCompactionThreshold).append(maxCompactionThreshold)
				.append(cfId).append(column_metadata)
				.append(compactionStrategyClass)
				.append(compactionStrategyOptions)
				.append(compressionParameters).append(bloomFilterFpChance)
				.append(memtableFlushPeriod).append(caching)
				.append(defaultTimeToLive).append(indexInterval)
				.append(speculativeRetry).append(populateIoCacheOnFlush)
				.append(isView) // Alex Santos
				.append(viewProperties)
				.toHashCode();
	}
	...
	public AbstractType<?> getValueValidator(ColumnDefinition columnDefinition) {
		return columnDefinition == null ? defaultValidator : columnDefinition
				.getValidator();
	}

	/** applies implicit defaults to cf definition. useful in updates */
	public static void applyImplicitDefaults(
		...
		// Alex Santos
		if (cf_def.is_view == null)
			cf_def.is_view = Collections.emptyMap();
		if (!cf_def.isSetIs_view())
			cf_def.setIs_view((Map<String, String>)new HashMap<String, String>());

		if (cf_def.view_properties == null)
			cf_def.view_properties = Collections.emptyMap();
		if (!cf_def.isSetView_properties())
			cf_def.setView_properties((Map<String, List<String>>)new HashMap<String, List<String>>());
	...
	public static CFMetaData fromThrift(org.apache.cassandra.thrift.CfDef cf_def)
		...
		try {
			CFMetaData newCFMD = new CFMetaData(cf_def.keyspace, cf_def.name,
					cfType, TypeParser.parse(cf_def.comparator_type),
			...
			if (cf_def.isSetCompaction_strategy_options())
				newCFMD.compactionStrategyOptions(new HashMap<String, String>(
						cf_def.compaction_strategy_options));
			...
			if (cf_def.isSetSpeculative_retry())
				newCFMD.speculativeRetry(SpeculativeRetry
						.fromString(cf_def.speculative_retry));
			if (cf_def.isSetPopulate_io_cache_on_flush())
				newCFMD.populateIoCacheOnFlush(cf_def.populate_io_cache_on_flush);
			// Alex Santos
			if (cf_def.isSetIs_view())
				newCFMD.isView(new HashMap<String, String>(cf_def.is_view));
			// Alex Santos
			if (cf_def.isSetView_properties())
				newCFMD.viewProperties(new HashMap<String, List<String>>(cf_def.view_properties));

			CompressionParameters cp = CompressionParameters
					.create(cf_def.compression_options);

			if (cf_def.isSetKey_validation_class()) {
				newCFMD.keyValidator(TypeParser
						.parse(cf_def.key_validation_class));
			}
			if (cf_def.isSetKey_alias()
					&& !(newCFMD.keyValidator instanceof CompositeType)) {
				newCFMD.column_metadata.put(cf_def.key_alias, ColumnDefinition
						.partitionKeyDef(cf_def.key_alias,
								newCFMD.keyValidator, null));
			}
			...
			return newCFMD
					...
					.columnMetadata(
							ColumnDefinition.fromThrift(cf_def.column_metadata,
									newCFMD.isSuper()))
					.compressionParameters(cp).updateCfDef();
	...
	public void reload() {
		...
		if (cfDefRow.cf == null || cfDefRow.cf.isEmpty())
			throw new RuntimeException(String.format(
					"%s not found in the schema definitions table.", ksName
							+ ":" + cfName));
	 ...
	 * 
	 * @throws ConfigurationException
	 *             if ks/cf names or cf ids didn't match
	 */
	public void apply(CFMetaData cfm) throws ConfigurationException {
		...
		// Alex Santos
		isView = cfm.isView;
		viewProperties = cfm.viewProperties;

		MapDifference<ByteBuffer, ColumnDefinition> columnDiff = Maps
				.difference(column_metadata, cfm.column_metadata);
	...
	public void validateCompatility(CFMetaData cfm)
		...
		if (!cfm.ksName.equals(ksName))
			throw new ConfigurationException(String.format(
					"Keyspace mismatch (found %s; expected %s)", cfm.ksName,
					ksName));
		...
		if (!cfm.cfId.equals(cfId))
			throw new ConfigurationException(String.format(
					"Column family ID mismatch (found %s; expected %s)",
					cfm.cfId, cfId));
	...
	public static void validateCompactionOptions(
			Class<? extends AbstractCompactionStrategy> strategyClass,
		...
		try {
			...
			Method validateMethod = strategyClass.getMethod("validateOptions",
					Map.class);
			Map<String, String> unknownOptions = (Map<String, String>) validateMethod
					.invoke(null, options);
			if (!unknownOptions.isEmpty())
				throw new ConfigurationException(String.format(
						"Properties specified %s are not understood by %s",
						unknownOptions.keySet(), strategyClass.getSimpleName()));
			...
			throw new ConfigurationException(
					"Failed to validate compaction options");
		} catch (Exception e) {
			throw new ConfigurationException(
					"Failed to validate compaction options");
		}
	...
	public static Class<? extends AbstractCompactionStrategy> createCompactionStrategy(
		...
		Class<AbstractCompactionStrategy> strategyClass = FBUtilities
				.classForName(className, "compaction strategy");
		if (!AbstractCompactionStrategy.class.isAssignableFrom(strategyClass))
			throw new ConfigurationException(
					String.format(
							"Specified compaction strategy class (%s) is not derived from AbstractReplicationStrategy",
							className));

		return strategyClass;
	}

	public AbstractCompactionStrategy createCompactionStrategyInstance(
			ColumnFamilyStore cfs) {
		try {
			Constructor<? extends AbstractCompactionStrategy> constructor = compactionStrategyClass
					.getConstructor(new Class[] { ColumnFamilyStore.class,
							Map.class // options
					});
	...
	public org.apache.cassandra.thrift.CfDef toThrift() {
		org.apache.cassandra.thrift.CfDef def = new org.apache.cassandra.thrift.CfDef(
				ksName, cfName);
		...
		// Alex Santos
		def.setIs_view(new HashMap<String, String>(isView));
		def.setView_properties(new HashMap<String, List<String>>(viewProperties));
		...
		// We only return the alias if only one is set since thrift don't know
		// about multiple key aliases
		if (partitionKeyColumns.size() == 1
				&& partitionKeyColumns.get(0) != null)
		...
		def.setCompaction_strategy_options(new HashMap<String, String>(
	 ...
	 */
	public ColumnDefinition getColumnDefinitionFromColumnName(
	...
	/**
	 * Convert a null index_name to appropriate default name according to column
	 * status
	 */
	public void addDefaultIndexNames() throws ConfigurationException {
		...
		if (cfId != null) {
			...
			for (Map.Entry<ByteBuffer, ColumnDefinition> entry : column_metadata
					.entrySet()) {
				...
				String oldIndexName = cfm.column_metadata.get(entry.getKey())
						.getIndexName();
				...
				if (newDef.getIndexName() != null
					...
					throw new ConfigurationException(
							"Can't modify index name: was '" + oldIndexName
									+ "' changed to '" + newDef.getIndexName()
									+ "'.");
		...
		for (ColumnDefinition column : column_metadata.values()) {
			if (column.getIndexType() != null && column.getIndexName() == null) {
				String baseName = getDefaultIndexName(cfName,
						getColumnDefinitionComparator(column), column.name);
	...
	public static String getDefaultIndexName(String cfName,
			AbstractType<?> comparator, ByteBuffer columnName) {
		return (cfName + "_" + comparator.getString(columnName) + "_idx")
				.replaceAll("\\W", "");
	}

	public Iterator<OnDiskAtom> getOnDiskIterator(DataInput in, int count,
			Descriptor.Version version) {
		return getOnDiskIterator(in, count, ColumnSerializer.Flag.LOCAL,
				(int) (System.currentTimeMillis() / 1000), version);
	}
	...
	public static boolean isNameValid(String name) {
		return name != null && !name.isEmpty()
				&& name.length() <= Schema.NAME_LENGTH && name.matches("\\w+");
	}
	...
	public CFMetaData validate() throws ConfigurationException {
		if (!isNameValid(ksName))
			throw new ConfigurationException(
					String.format(
							"Keyspace name must not be empty, more than %s characters long, or contain non-alphanumeric-underscore characters (got \"%s\")",
							Schema.NAME_LENGTH, ksName));
		if (!isNameValid(cfName))
			throw new ConfigurationException(
					String.format(
							"ColumnFamily name must not be empty, more than %s characters long, or contain non-alphanumeric-underscore characters (got \"%s\")",
							Schema.NAME_LENGTH, cfName));

		if (cfType == null)
			throw new ConfigurationException(String.format(
					"Invalid column family type for %s", cfName));

		if (comparator instanceof CounterColumnType)
			throw new ConfigurationException(
					"CounterColumnType is not a valid comparator");
		if (keyValidator instanceof CounterColumnType)
			throw new ConfigurationException(
					"CounterColumnType is not a valid key validator");
				...
				if (!(def.getValidator() instanceof CounterColumnType))
					throw new ConfigurationException(
							"Cannot add a non counter column ("
									+ getColumnDefinitionComparator(def)
											.getString(def.name)
									+ ") in a counter column family");
		} else {
			for (ColumnDefinition def : column_metadata.values())
				if (def.getValidator() instanceof CounterColumnType)
					throw new ConfigurationException(
							"Cannot add a counter column ("
									+ getColumnDefinitionComparator(def)
											.getString(def.name)
		...
		for (ColumnDefinition c : column_metadata.values()) {
			...
			try {
				...
				throw new ConfigurationException(String.format(
						"Column name %s is not valid for comparator %s",
			...
			if (c.getIndexType() == null) {
				...
				if (c.getIndexType() == IndexType.CUSTOM) {
					if (c.getIndexOptions() == null
							|| !c.getIndexOptions().containsKey(
									SecondaryIndex.CUSTOM_INDEX_OPTION_NAME))
						throw new ConfigurationException(
								"Required index option missing: "
										+ SecondaryIndex.CUSTOM_INDEX_OPTION_NAME);
				}

				// This method validates the column metadata but does not
				// intialize the index
		...
		if (bloomFilterFpChance != null && bloomFilterFpChance == 0)
			throw new ConfigurationException(
					"Zero false positives is impossible; bloom filter false positive chance bffpc must be 0 < bffpc <= 1");
	...
	private static void validateAlias(ColumnDefinition alias, String msg)
			throws ConfigurationException {
		if (alias != null) {
			try {
	 ...
	 * 
	 * @param newState
	 *            The new metadata (for the same CF)
	...
	public RowMutation toSchemaUpdate(CFMetaData newState,
			long modificationTimestamp, boolean fromThrift) {
		...
		MapDifference<ByteBuffer, ColumnDefinition> columnDiff = Maps
				.difference(column_metadata, newState.column_metadata);

		// columns that are no longer needed
		for (ColumnDefinition cd : columnDiff.entriesOnlyOnLeft().values()) {
			// Thrift only knows about the REGULAR ColumnDefinition type, so
			// don't consider other type
	...
	public RowMutation dropFromSchema(long timestamp) {
		...
		// Alex Santos
		cf.addColumn(DeletedColumn.create(ldt, timestamp, cfName, "is_view"));
		cf.addColumn(DeletedColumn.create(ldt, timestamp, cfName, "view_properties"));
		cf.addColumn(DeletedColumn.create(ldt, timestamp, cfName,
				"gc_grace_seconds"));
		cf.addColumn(DeletedColumn.create(ldt, timestamp, cfName,
				"default_validator"));
		...
		cf.addColumn(DeletedColumn
				.create(ldt, timestamp, cfName, "key_aliases"));
		cf.addColumn(DeletedColumn.create(ldt, timestamp, cfName,
		...
		cf.addColumn(DeletedColumn
				.create(ldt, timestamp, cfName, "value_alias"));
		cf.addColumn(DeletedColumn.create(ldt, timestamp, cfName,
				"column_aliases"));
		cf.addColumn(DeletedColumn.create(ldt, timestamp, cfName,
	...
	private void toSchemaNoColumns(RowMutation rm, long timestamp) {
		...
		if (isSuper()) {
			...
			// separatly, otherwise
			// we won't know at deserialization if the subcomparator should be
			// taken into account
			// TODO: we should implement an on-start migration if we want to get
			// rid of that.
			CompositeType ct = (CompositeType) comparator;
		...
		cf.addColumn(comment == null ? DeletedColumn.create(ldt, timestamp,
				cfName, "comment") : Column.create(comment, timestamp, cfName,
				"comment"));
		cf.addColumn(Column.create(readRepairChance, timestamp, cfName,
				"read_repair_chance"));
		cf.addColumn(Column.create(dcLocalReadRepairChance, timestamp, cfName,
				"local_read_repair_chance"));
		...
		cf.addColumn(Column.create(populateIoCacheOnFlush, timestamp, cfName,
				"populate_io_cache_on_flush"));
		// Alex Santos
		cf.addColumn(Column.create(json(isView), timestamp, cfName,
				"is_view"));
		cf.addColumn(Column.create(json(viewProperties), timestamp, cfName,
				"view_properties"));
		...
		cf.addColumn(Column.create(keyValidator.toString(), timestamp, cfName,
				"key_validator"));
		...
		cf.addColumn(bloomFilterFpChance == null ? DeletedColumn.create(ldt,
				timestamp, cfName, "bloomFilterFpChance") : Column.create(
				bloomFilterFpChance, timestamp, cfName,
				"bloom_filter_fp_chance"));
		...
		cf.addColumn(Column.create(
				json(compressionParameters.asThriftOptions()), timestamp,
				cfName, "compression_parameters"));
		cf.addColumn(Column.create(json(compactionStrategyOptions), timestamp,
				cfName, "compaction_strategy_options"));
		...
		cf.addColumn(compactValueColumn == null ? DeletedColumn.create(ldt,
				timestamp, cfName, "value_alias") : Column.create(
				compactValueColumn.name, timestamp, cfName, "value_alias"));
	}
	...
	static CFMetaData fromSchemaNoColumns(UntypedResultSet.Row result) {
		try {
			CFMetaData cfm = new CFMetaData(result.getString("keyspace_name"),
					...
					result.has("subcomparator") ? TypeParser.parse(result
							.getString("subcomparator")) : null);

			if (result.has("id"))// try to identify if ColumnFamily Id is old
									// style (before C* 1.2) and add old -> new
									// mapping if so
				Schema.instance
						.addOldCfIdMapping(result.getInt("id"), cfm.cfId);

			cfm.readRepairChance(result.getDouble("read_repair_chance"));
			cfm.dcLocalReadRepairChance(result
					.getDouble("local_read_repair_chance"));
			cfm.replicateOnWrite(result.getBoolean("replicate_on_write"));
			cfm.gcGraceSeconds(result.getInt("gc_grace_seconds"));
			cfm.defaultValidator(TypeParser.parse(result
					.getString("default_validator")));
			cfm.keyValidator(TypeParser.parse(result.getString("key_validator")));
			cfm.minCompactionThreshold(result
					.getInt("min_compaction_threshold"));
			cfm.maxCompactionThreshold(result
					.getInt("max_compaction_threshold"));
			if (result.has("comment"))
				cfm.comment(result.getString("comment"));
			// We need support the old key_alias for compatibility sake
			if (result.has("bloom_filter_fp_chance"))
				cfm.bloomFilterFpChance(result
						.getDouble("bloom_filter_fp_chance"));
			if (result.has("memtable_flush_period_in_ms"))
				cfm.memtableFlushPeriod(result
						.getInt("memtable_flush_period_in_ms"));
			...
			if (result.has("speculative_retry"))
				cfm.speculativeRetry(SpeculativeRetry.fromString(result
						.getString("speculative_retry")));
			cfm.compactionStrategyClass(createCompactionStrategy(result
					.getString("compaction_strategy_class")));
			cfm.compressionParameters(CompressionParameters
					.create(fromJsonMap(result
							.getString("compression_parameters"))));
			cfm.compactionStrategyOptions(fromJsonMap(result
					.getString("compaction_strategy_options")));
			if (result.has("index_interval"))
				cfm.indexInterval(result.getInt("index_interval"));
			if (result.has("populate_io_cache_on_flush"))
				cfm.populateIoCacheOnFlush(result
						.getBoolean("populate_io_cache_on_flush"));
			// Alex Santos
			cfm.isView(fromJsonMap(result.getString("is_view")));

			//Alex Santos: Convert info to Map of Strings, then convert the String to an ArrayList
			if (result.has("view_properties")) {
				HashMap<String, String> aux_map = (HashMap<String, String>) fromJsonMap(result.getString("view_properties"));
				HashMap<String, List<String>> view_properties = new HashMap<String, List<String>>();

				for (Map.Entry<String, String> entry : aux_map.entrySet())
            	{
            		Object properties_abstract = (Object) entry.getValue();
            		ArrayList<String> properties_list = (ArrayList<String>) properties_abstract;
             		view_properties.put(entry.getKey(), properties_list);
            	}

            	cfm.viewProperties((HashMap<String, List<String>>) view_properties);
			}
			//cfm.viewProperties(fromJsonMap(result.getList("view_properties")));
			 ...
			 * However, for upgrade sake we need to still be able to read those
			 * old values. Moreover, we cannot easily remove those old columns
			 * once "converted" to column_metadata because that would screw up
			 * nodes that may not have upgraded. So for now we keep the both
			 * info and in sync, even though its redundant. In other words, the
			 * ColumnDefinition the following lines add may be replaced later
			 * when ColumnDefinition.fromSchema is called but that's ok.
			 */
			if (result.has("key_aliases"))
				cfm.addColumnMetadataFromAliases(
						aliasesFromStrings(fromJsonList(result
								.getString("key_aliases"))), cfm.keyValidator,
						ColumnDefinition.Type.PARTITION_KEY);
			else if (result.has("key_alias"))
				cfm.addColumnMetadataFromAliases(Collections
						.<ByteBuffer> singletonList(result
								.getBytes("key_alias")), cfm.keyValidator,
						ColumnDefinition.Type.PARTITION_KEY);

			cfm.addColumnMetadataFromAliases(
					aliasesFromStrings(fromJsonList(result
							.getString("column_aliases"))), cfm.comparator,
					ColumnDefinition.Type.CLUSTERING_KEY);

			if (result.has("value_alias"))
				cfm.addColumnMetadataFromAliases(Collections
						.<ByteBuffer> singletonList(result
								.getBytes("value_alias")),
	...
	public void addColumnMetadataFromAliases(List<ByteBuffer> aliases,
			AbstractType<?> comparator, ColumnDefinition.Type type) {
		if (comparator instanceof CompositeType) {
			CompositeType ct = (CompositeType) comparator;
			for (int i = 0; i < aliases.size(); ++i) {
				if (aliases.get(i) != null)
					column_metadata.put(aliases.get(i), new ColumnDefinition(
							aliases.get(i), ct.types.get(i), i, type));
			}
		} else {
			assert aliases.size() <= 1;
			if (!aliases.isEmpty() && aliases.get(0) != null)
				column_metadata.put(aliases.get(0), new ColumnDefinition(
						aliases.get(0), comparator, null, type));
		}
		updateCfDef();
	}
	...
	public static CFMetaData fromSchema(UntypedResultSet.Row result) {
		...
		Row serializedColumnDefinitions = ColumnDefinition.readSchema(
				cfDef.ksName, cfDef.cfName);
		return addColumnDefinitionSchema(cfDef, serializedColumnDefinitions);
	}

	private static CFMetaData fromSchema(Row row) {
		UntypedResultSet.Row result = QueryProcessor.resultify(
				"SELECT * FROM system.schema_columnfamilies", row).one();
	...
	private String aliasesToJson(List<ColumnDefinition> rawAliases) {
		...
		for (ColumnDefinition rawAlias : rawAliases)
			aliases.add(rawAlias == null ? null : UTF8Type.instance
					.compose(rawAlias.name));
		return json(aliases);
	}
	...
	private static List<ByteBuffer> aliasesFromStrings(List<String> aliases) {
		...
		for (String alias : aliases)
			rawAliases.add(alias == null ? null : UTF8Type.instance
					.decompose(alias));
		return rawAliases;
	}
	 ...
	 * 
	 * @throws ConfigurationException
	 *             if any of the attributes didn't pass validation
	 */
	...
	public AbstractType<?> getComponentComparator(Integer componentIndex,
			ColumnDefinition.Type type) {
		switch (type) {
		case REGULAR:
			AbstractType<?> cfComparator = cfType == ColumnFamilyType.Super ? ((CompositeType) comparator).types
					.get(1) : comparator;
	...
	// Package protected for use by tests
	static CFMetaData addColumnDefinitionSchema(CFMetaData cfDef,
			Row serializedColumnDefinitions) {
		for (ColumnDefinition cd : ColumnDefinition.fromSchema(
				serializedColumnDefinitions, cfDef))
	...
	public void addColumnDefinition(ColumnDefinition def)
		...
		if (column_metadata.containsKey(def.name))
			throw new ConfigurationException(
					String.format(
							"Cannot add column %s, a column with the same name already exists",
							getColumnDefinitionComparator(def).getString(
									def.name)));

		addOrReplaceColumnDefinition(def);
	}

	// This method doesn't check if a def of the same name already exist and
	// should only be used when we
	...
	public void renameColumn(ByteBuffer from, String strFrom, ByteBuffer to,
		...
		if (def == null)
			throw new InvalidRequestException(String.format(
					"Cannot rename unknown column %s in table %s", strFrom,
					cfName));

		if (column_metadata.get(to) != null)
			throw new InvalidRequestException(
					String.format(
							"Cannot rename column %s to %s in table %s; another column of that name already exist",
							strFrom, strTo, cfName));

		if (def.type == ColumnDefinition.Type.REGULAR)
			throw new InvalidRequestException(String.format(
					"Cannot rename non PRIMARY KEY part %s", strFrom));
	...
	private CFMetaData updateCfDef() {
		/*
		 * TODO: There is definitively some repetition between the CQL3 metadata
		 * stored in this object (partitionKeyColumns, ...) and the one stored
		 * in CFDefinition. Ultimately, we should probably merge both. However,
		 * there is enough details to fix that it's worth doing that in a
		 * separate issue.
		 */
		rebuildCQL3Metadata();
	...
	private void rebuildCQL3Metadata() {
		List<ColumnDefinition> pkCols = nullInitializedList(keyValidator
				.componentsCount());
		int nbCkCols = isDense(comparator, column_metadata.values()) ? comparator
				.componentsCount() : comparator.componentsCount()
				- (hasCollection() ? 2 : 1);
		...
		for (ColumnDefinition def : column_metadata.values()) {
			...
			case PARTITION_KEY:
				...
				pkCols.set(def.componentIndex == null ? 0 : def.componentIndex,
						def);
				break;
			case CLUSTERING_KEY:
				assert !(def.componentIndex == null && comparator instanceof CompositeType);
				ckCols.set(def.componentIndex == null ? 0 : def.componentIndex,
						def);
				break;
			case REGULAR:
				regCols.add(def);
		...
		// Now actually assign the correct value. This is not atomic, but then
		// again, updating CFMetaData is never atomic anyway.
	 ...
	 * clustering column, i.e. no component is used to store a regular column
	 * names. In other words, non-composite static "thrift" and CQL3 CF are
	 * *not* dense. Note that his method is only used by rebuildCQL3Metadata.
	 * Once said metadata are built, finding if a CF is dense amounts more
	 * simply to check if clusteringKeyColumns.size() ==
	 * comparator.componentsCount().
	 */
	private static boolean isDense(AbstractType<?> comparator,
			Collection<ColumnDefinition> defs) {
		/*
		 * This is a bit subtle to compute because of thrift upgrades. A CQL3 CF
		 * will have all it's column metadata set up from creation, so checking
		 * isDense should just be looking the ColumnDefinition of type
		 * CLUSTERING_KEY having the biggest componentIndex and comparing that
		 * to comparator.componentsCount. However, thrift CF will have no or
		 * only some (through ALTER RENAME) metadata set and we still need to
		 * make our best effort at finding whether it is intended as a dense CF
		 * or not.
		 */

		// First, we compute the number of clustering columns metadata actually
		// defined (and
		// whether there is some "hole" in the metadata)
		boolean[] definedClusteringKeys = new boolean[comparator
				.componentsCount()];
		boolean hasRegular = false;
		for (ColumnDefinition def : defs) {
			switch (def.type) {
			case CLUSTERING_KEY:
				definedClusteringKeys[def.componentIndex == null ? 0
						: def.componentIndex] = true;
				break;
			case REGULAR:
				hasRegular = true;
				break;
		...
		if (comparator instanceof CompositeType) {
			List<AbstractType<?>> types = ((CompositeType) comparator).types;
			/*
			 * There was no real way to define a non-dense composite CF in
			 * thrift (the ColumnDefinition.componentIndex is not exposed), so
			 * consider dense anything that don't look like a CQL3 created CF.
			 * 
			 * Note that this is not perfect: if someone upgrading from thrift
			 * "renames" all but the last column alias, the cf will be
			 * considered "sparse" and he will be stuck with that even though
			 * that might not be what he wants. But the simple workaround is for
			 * that user to rename all the aliases at the same time in the first
			 * place.
			 */
			...
			/*
			 * For non-composite, we only need to "detect" case where the CF is
			 * clearly used as static. For that, just check if we have regular
			 * columns metadata sets up and no defined clustering key.
			 */
	...
	public boolean isThriftCompatible() {
		// Super CF are always "thrift compatible". But since they may have defs
		// with a componentIndex != null,
	...
	public String toString() {
		return new ToStringBuilder(this)
				...
				.append("isView", isView) // Alex Santos
				.append("viewProperties", viewProperties) // Alex Santos
				.toString();
	}
}
