...
public class CliClient
{
    ...
    public void executeCLIStatement(String statement) throws CharacterCodingException, TException, TimedOutException, NotFoundException, NoSuchFieldException, InvalidRequestException, UnavailableException, InstantiationException, IllegalAccessException, ClassNotFoundException
    {
        ...
        try
        {
            switch (tree.getType())
            {
                ...
                //Alex Santos
                case CliParser.NODE_CREATE_JOIN:
                    System.out.println("CREATING JOIN " + tree.getChild(0) + "... " + tree.getChild(1) + "[" + tree.getChild(2) + "] -> " + tree.getChild(3));
                    executeAddJoin(tree);
                    break;
                case CliParser.NODE_CREATE_INDEX_VIEW:
                    //System.out.println("CREATING INDEX VIEW " + tree.getChild(0) + "... " + tree.getChild(1) + "[" + tree.getChild(2) + "]");
                    executeAddIndexView(tree);
                    break;
    ...
    private CfDef updateCfDefAttributes(Tree statement, CfDef cfDefToUpdate)
    {
        ...
        for (int i = 1; i < statement.getChildCount(); i += 2)
        {
            ...
            //Alex Santos
            case is_view:
               // cfDef.setis_view(getStrategyOptionsFromTree(statement.getChild(i+1)));
                break;
            default:
                //must match one of the above or we'd throw an exception at the valueOf statement above.
                assert(false);
    ...
    //Alex Santos: CREATE JOIN <CF> OF <CF>.<COL> AND <CF>
    private void executeAddJoin(Tree statement)  throws TException, SchemaDisagreementException, InvalidRequestException, NotFoundException
    {
        if (!CliMain.isConnected() || !hasKeySpace())
            return;

        System.out.println("DEBUG: executeAddJoin");

        String view = CliUtils.unescapeSQLString(statement.getChild(0).getText());
        String cfA  = CliCompiler.getColumnFamily(statement.getChild(1).getText(), currentCfDefs());
        String cfB  = CliCompiler.getColumnFamily(statement.getChild(3).getText(), currentCfDefs());
        String rawColumName = CliUtils.unescapeSQLString(statement.getChild(2).getText());

        CfDef cfDefA = getCfDef(cfA);

        ByteBuffer columnName = columnNameAsBytes(rawColumName, cfDefA);

        /* Check if join column exists */
        /* boolean foundColumn = false;

        for (ColumnDef column : cfDefA.getColumn_metadata())
        {
            if (column.name.equals(columnName))
            {
                foundColumn = true;
		...
		*/
        ...
        /* Join View Configuration */
        HashMap<String, String> opts = new HashMap<String, String>();
        opts.put("TYPE", "join");
        opts.put("CF_A", cfA);
        opts.put("CF_B", cfB);
        opts.put("COL_A", rawColumName);
        
        /* Create Materialised View */     
        CfDef newCfDef = new CfDef(keySpace, view);
        newCfDef.setIs_view(opts);
        newCfDef.comparator_type = cfDefA.comparator_type;
        newCfDef.key_validation_class = cfDefA.key_validation_class;
        newCfDef.default_validation_class = cfDefA.default_validation_class;
        
        String mySchemaVersion = thriftClient.system_add_column_family(newCfDef);
        sessionState.out.println(mySchemaVersion);
        keyspacesMap.put(keySpace, thriftClient.describe_keyspace(keySpace));   

        System.out.println(String.format("%s[%s] -> %s", cfA, rawColumName, cfB));
    }
    
    //Alex Santos: CREATE INDEX VIEW <CF_Av> OF <CF_A>[<COL_v>+] (AND CF_X[<COL_v>+])*
    private void executeAddIndexView(Tree statement)  throws TException, SchemaDisagreementException, InvalidRequestException, NotFoundException
    {
        if (!CliMain.isConnected() || !hasKeySpace())
            return;
        ...
        int key = 0;
        while (++key < statement.getChildCount()) {
            Boolean is_cf = statement.getChild(key).getType() == CliParser.COLUMN_FAMILY;
            if (is_cf) {
                System.out.println("** " + statement.getChild(key).getChild(0).getText());
                cf.add(CliCompiler.getColumnFamily(statement.getChild(key).getChild(0).getText(), currentCfDefs()));
            } else {
                System.out.println("** " + statement.getChild(key).getText());
                keys.add(CliUtils.unescapeSQLString(statement.getChild(key).getText()));
            }
        }

       /*int key = 2;
        while (key < statement.getChildCount()) {
            rawColumName = CliUtils.unescapeSQLString(statement.getChild(key++).getText());
            System.out.println("** " + rawColumName);
            keys.add(rawColumName);
        }
        */

        CfDef cfDefA = getCfDef(cf.get(0));

        //ByteBuffer columnName = columnNameAsBytes(rawColumName, cfDefA); 
        
        // Index View Configuration //
        HashMap<String, List<String>> opts = new HashMap<String, List<String>>();
        ...
        // Create Materialised View //     
        CfDef newCfDef = new CfDef(keySpace, view);
        newCfDef.setView_properties(opts);
        newCfDef.comparator_type = cfDefA.comparator_type;
        newCfDef.key_validation_class = cfDefA.key_validation_class;
        newCfDef.default_validation_class = cfDefA.default_validation_class;
        ...
        /* System.out.println(String.format("%s[%s]", cfA, rawColumName));*/
    ...
    private void showColumnFamily(PrintStream output, CfDef cfDef)
    {
        ...
        //Alex Santos
        if (!cfDef.is_view.isEmpty())
        {
            StringBuilder cOptions = new StringBuilder();

            cOptions.append("{");

            Map<String, String> joinopts = cfDef.is_view;

            int i = 0, size = joinopts.size();

            for (Map.Entry<String, String> entry : joinopts.entrySet())
            {
                cOptions.append(CliUtils.quote(entry.getKey())).append(" : ").append(CliUtils.quote(entry.getValue()));
            ...
            writeAttrRaw(output, false, "is_view", cOptions.toString());
        }

        writeAttrRaw(output, false, "view_properties", cfDef.view_properties.size()+"");
        if (!cfDef.view_properties.isEmpty())
        {
            StringBuilder cOptions = new StringBuilder();

            cOptions.append("{");

            Map<String, List<String>> viewopts = cfDef.view_properties;

            int i = 0, size = viewopts.size();

            for (Map.Entry<String, List<String>> entry : viewopts.entrySet())
            {
                cOptions.append(CliUtils.quote(entry.getKey())).append(" : [");

                for (String a : (List<String>)entry.getValue())
                    cOptions.append(CliUtils.quote(a) + " ");
                cOptions.append("]");
            ...
            writeAttrRaw(output, false, "view_properties", cOptions.toString());
        }
    ...
    private void describeColumnFamily(KsDef ks_def, CfDef cf_def, NodeProbe probe) throws TException
    {
        ...
        sessionState.out.printf("      Join View: %b%n", cf_def.is_view); //Alex Santos
