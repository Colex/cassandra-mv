...
statement
    ...
    //Alex Santos
    | addJoinStatement
    | addIndexViewStatement
    | addIndexViewStatement2
...
//Alex Santos
addJoinStatement
    : CREATE JOIN matView=columnFamily OF famA=columnFamily '.' keyA=rowKey 'AND' famB=columnFamily
        -> ^(NODE_CREATE_JOIN $matView $famA $keyA $famB)
    ;

addIndexViewStatement
    : CREATE INDEX VIEW columnFamily OF singleColumnFamilyExpr '[' rowKey (',' rowKey)* ']' ('AND' singleColumnFamilyExpr '[' rowKey (',' rowKey)* ']')*
        -> ^(NODE_CREATE_INDEX_VIEW columnFamily singleColumnFamilyExpr+ rowKey+)
    ;

addIndexViewStatement2
	: CREATE INDEX VIEW2 viewName=columnFamily OF colFam=columnFamily '.' rowKey
		-> ^(NODE_CREATE_INDEX_VIEW $viewName $colFam rowKey)
	;
...
singleColumnFamilyExpr
  : entityName
    -> ^(COLUMN_FAMILY entityName)
  ;
